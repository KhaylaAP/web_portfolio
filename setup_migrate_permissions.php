<?php
/**
 * Setup script to add Migrate page and permissions
 * This ensures the admin user has access to the migration management page
 */
...
try {
    ...
    // 1. Add the 'migrate' page if it doesn't exist
    $check_page = "SELECT id FROM pages WHERE page_code = 'migrate'";
    $stmt = $conn->query($check_page);
    
    if ($stmt->rowCount() === 0) {
        $insert_page = "INSERT INTO pages (page_code, page_name, description, url) 
                       VALUES ('migrate', 'Database Migrations', 'Manage database migrations and schema updates', '/admin/migrate/index.php')";
        $conn->exec($insert_page);
        echo "✓ Migrate page added to database\n";
    } else {
        echo "✓ Migrate page already exists\n";
    }
    
    // 2. Add migration permissions if they don't exist
    $permissions_to_add = [
        ['view_migrate', 'View Migrations', 'READ'],
        ['run_migrate', 'Run Migrations', 'CREATE']
    ];
    ...
    // 3. Grant admin user all migration permissions
    ...
    if ($admin) {
        ...
        $page_query = "SELECT id FROM pages WHERE page_code = 'migrate'";
    ...
    echo "\n✅ Migrate page and permissions setup complete!\n";
